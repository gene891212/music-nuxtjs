        ...
        <div class="flex items-center gap-8 flex-1">
          ...
          <div class="flex-1 max-w-xl">
            <div class="relative">
              ...
              <input v-model="searchQuery" type="text" placeholder="搜尋歌曲、專輯、藝人或 Podcast" class="w-full pl-12 pr-4 py-2 border border-gray-200 bg-gray-100 rounded-full focus:outline-none focus:ring-2 focus:ring-red-500 focus:bg-white">
            </div>
          </div>
        </div>
          ...
          <template v-else>
            ...
            <div class="relative">
              ...
              <div v-if="showUserMenu" class="absolute right-0 mt-2 w-48 bg-white rounded-lg shadow-lg border py-1">
                ...
                <button @click="handleSignOut" class="w-full text-left px-4 py-2 text-sm text-red-600 hover:bg-gray-100">登出</button>
              </div>
            </div>
          </template>
        </div>
